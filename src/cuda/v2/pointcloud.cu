...
void pc_reserve(PointCloud *c, int n)
{
	...
	while (cap < n) cap *= 2;
	c->x = (float *)realloc(c->x, (size_t)cap * sizeof *c->x);
	c->y = (float *)realloc(c->y, (size_t)cap * sizeof *c->y);
	c->z = (float *)realloc(c->z, (size_t)cap * sizeof *c->z);
...
void pc_morton_order(const PointCloud *src, PointCloud *dst)
{
	...
	MortonKey *keys = (MortonKey *)malloc((size_t)src->n * sizeof *keys);
