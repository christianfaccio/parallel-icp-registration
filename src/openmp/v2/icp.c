 ...
 *
 * Differs from openmp/v0 only in stage 3: the centroid + cross-covariance
 * reduction (previously serial, ~8% of total time once NN search was sped
 * up) is now a second `#pragma omp parallel for reduction(...)` region,
 * using OpenMP array-section reductions for cs/ct/H.
 */
            ...
            const ICPParams *prm, ICPResult *res)
{
	...
	for (; it < prm->max_iters; it++) {
		...
		long   m = 0;
		#pragma omp parallel for reduction(+:cs[:3], ct[:3], m)
		for (int i = 0; i < cur.n; i++) {
			double mask = (double)(match[i] >= 0);
		...
		double sse  = 0.0;
		#pragma omp parallel for reduction(+:H[:9], sse)
		for (int i = 0; i < cur.n; i++) {
			double mask = (double)(match[i] >= 0);
			int j = (match[i] >= 0) ? match[i] : 0;	// safe
